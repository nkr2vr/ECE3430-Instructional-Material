...
// Global variable
extern char char_has_not_been_received;
...
void InitializeUART()
{
	...
	USCIA0_RXD_P1SEL0 |= USCIA0_RXD_BIT;
	USCIA0_RXD_P1SEL1 &= ~USCIA0_RXD_BIT;

	USCIA0_TXD_P1SEL0 |= USCIA0_TXD_BIT;
	USCIA0_TXD_P1SEL1 &= ~USCIA0_TXD_BIT;
	...
	// Use SMCLK
	UCA0CTL0 |= EUSCI_A_CTLW0_SSEL__SMCLK;
	 ...
	 */
	UCA0CTL1 = EUSCI_A_CTLW1_GLIT_0;
	
	UCA0BR0 = UCA0BR0_VALUE;
	UCA0MCTLW = UCOS16 + (UCBRS_M & 0x49) + (UCBRF_M & 0x01);
	/*
...
{ 
	 ...
	 */
	for (character_index = 0; *(pt+character_index); character_index++) {
		WriteCharacterUART(*(pt+character_index));
	}
}
